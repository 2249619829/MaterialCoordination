...
这一阶段补上后端项目展示里很重要的性能证据。项目保留 JMeter 脚本、压测数据准备脚本和压测报告，明确说明这是本地单机压测，不把结果夸大成生产能力。
...
这一阶段把项目从“能跑”整理成“能讲”。文档覆盖 README、架构、API、启动、压测、演示路线和 Windows 迁移说明。
...
- `docs/performance.md`：JMeter 压测结果和瓶颈分析。
