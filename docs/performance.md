...
项目后续主推的物流高并发链路是司机抢运输单：

```http
POST /api/transport-orders/{orderId}/claim
```

该链路用于模拟供应商确认供货后，多个司机同时抢接同一个待分配运输任务。系统先通过 Redis Lua 预占订单运力名额，再发送 RabbitMQ 消息，消费者使用 Redisson 锁和 MySQL 条件更新异步绑定司机。
...
> 供应商确认供货后，订单会进入待分配运输订单池。司机抢单接口不会直接让所有请求打 MySQL，而是先用 Redis Lua 原子判断订单是否还有运力名额、司机是否重复抢单；预占成功后发送 RabbitMQ，由消费者异步绑定司机、更新推送记录并写订单时间线。消费者侧使用 Redisson 锁和 MySQL 条件更新兜底，保证重复消息不会导致重复接单。
