...
该平台面向供应链物资协同和自然灾害应急采购场景。自然灾害发生后，采购方需要快速找到可履约供应商，供应商需要维护可供物资和资质能力，司机需要接收运输订单并完成配送。平台通过缓存、消息队列、分布式锁和 Redis 多种数据结构，解决高并发抢购、订单推送、履约评价和应急调度问题。
...
> 订单创建先写 Redis 临时订单，再发 RabbitMQ，消费者异步落库。供应商确认供货时用 MySQL 条件更新扣减库存，订单进入司机大厅并按关注关系创建推送记录。司机侧既可以查看关注关系推送过来的订单，也可以主动从订单大厅拉取，这就是推拉结合。
...
flowchart TD
    ...
    B --> C{"评价目标是供应商?"}
    C -- 是 --> D["重新计算供应商平均分"]
    D --> E["更新 supplier_profile.rating_score"]
    E --> F["写 Redis ZSet ranking:supplier:fulfillment"]
    C -- 否 --> G["只保留评价记录"]
    F --> H["前端排行榜刷新"]
```
...
> 平台围绕订单做三方评价。供应商被评价后会重新计算履约评分，并同步到 Redis ZSet，这样排行榜读取只需要从 ZSet 取 TopN，适合高频展示。
...
> Redis 不只是缓存。ZSet 用于排行榜，GEO 用于附近供应商调度，BitMap 用于海量司机签到状态。这样既能体现业务建模，也能体现对 Redis 数据结构的理解。
...
7. 推拉结合：推送记录给关注关系司机，司机也可以主动拉取订单大厅。
8. Redis 扩展：ZSet 排行榜、GEO 附近供应商、BitMap 司机出勤。
9. 可靠性：RabbitMQ 死信队列、异常观测、补偿接口。
10. 前端展示：三角色页面不同，能直观看到业务隔离和核心链路。
